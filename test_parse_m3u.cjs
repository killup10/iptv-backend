...
const filePath = __dirname + '/test_dragonball.m3u';
...
for (let i=0;i<lines.length;i++){
  ...
  // use first comma after #EXTINF to extract the title (safer when title contains commas)
  const tmatch = line.match(/#EXTINF:[^,]*,(.*)$/i);
  let titlePart = tmatch ? tmatch[1].trim() : line.substring(line.lastIndexOf(',')+1).trim();
